...
public class GroupeP implements Groupe {
    ...
    public GroupeP(String name, int min, int max){
        ...
        try {
            ...
            ps = this.connexion.getConnection().prepareStatement("INSERT INTO GROUPE (ID_GROUPE, NOM_GROUPE, MIN, MAX, TYPE_GROUPE) VALUES(?,?,?,?,?)");
            
            ps.setInt(1,this.id);
            ps.setString(2, this.name);
            ps.setInt(3, this.min);
            ps.setInt(4, this.max);
            ps.setString(5, "Tous les étudiants");

            ps.executeQuery();

            System.out.println("Groupe inseré");
    ...
    //     // this.membresDuGroupe=new LinkedHashSet<Etudiant>();
        ...
        try {
            ...
            PreparedStatement ps = this.connexion.getPrepare();
            ps = this.connexion.getConnection().prepareStatement("INSERT INTO GROUPE(NOM_GROUPE,MIN,MAX,TYPE_GROUPE) VALUES(?,?,?,?");
            
            ps.setString(1, this.name);
            ps.setInt(2, this.min);
            ps.setInt(3, this.max);
            ps.setString(4, "FREE");
